...
let cachedAI: GoogleGenAI | null | undefined = undefined;

const getAIClient = (): GoogleGenAI | null => {
  // Si ya lo intentamos inicializar, devolvemos el resultado guardado
  if (cachedAI !== undefined) return cachedAI;

  const apiKey = import.meta.env.VITE_GOOGLE_API_KEY;

  // Si no hay API key, no rompemos nada
  if (!apiKey || apiKey.includes("DUMMY") || apiKey === "TU_API_KEY_DE_GEMINI_AQUI") {
    console.warn("Google API Key no configurada. La IA estará desactivada.");
    cachedAI = null;
    return null;
  }
...
export const generateOvertimeAnalysis = async (
  ...
  try {
    ...
    console.error("Error AI (horas extra):", error);
...
export const generateInventoryEmail = async (
  ...
  try {
    ...
    console.error("Error AI (inventario):", error);
...
export const generateFinancialReport = async (
  ...
  try {
    ...
    console.error("Error AI (finanzas):", error);
...
export const generateBudgetAnalysis = async (
  ...
  try {
    ...
    console.error("Error AI (presupuesto):", error);
    return null;
  }
};
