...
export const InventoryManager: React.FC<InventoryManagerProps> = ({
  ...
  // ✅ Generador de “informe email” SIN IA, totalmente local
  const handleGenerateEmail = (session: InventorySession) => {
    setGeneratingReportId(session.id);
    setReportText(null);

    const fecha = new Date(session.date).toLocaleDateString('es-AR');
    const apertura = session.startTime;
    const cierre = session.endTime;
    const abiertoPor = session.openedBy;
    const cerradoPor = session.closedBy;

    const lineasItems = session.data
      .map((d) => {
        const item = items.find((i) => i.id === d.itemId);
        if (!item) return null;
        const consumo = d.consumption ?? (d.initial ?? 0) - (d.final ?? 0);
        return `• ${item.name} (${item.unit}) – Inicial: ${d.initial ?? 0}, Final: ${
          d.final ?? 0
        }, Consumo: ${consumo.toFixed(2)} ${item.unit}`;
      })
      .filter(Boolean)
      .join('\n');

    const cuerpo = `
Asunto: Reporte de inventario – ${fecha}

Buenas,

Comparto el resumen del inventario correspondiente al turno de cocina.

Fecha: ${fecha}
Apertura: ${apertura} (${abiertoPor})
Cierre: ${cierre} (${cerradoPor})

Detalle de consumos:

${lineasItems || '- Sin datos de consumos registrados.'}

Saludos,
${userName}
`.trim();

    setReportText(cuerpo);
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 dark:text-sushi-muted mt-2">
            Control de stock inicial y final por turno.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-sushi-dark border border-gray-200 dark:border-white/5 rounded-xl p-6 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">
              {currentSession
                ? 'Cierre de Servicio (Final)'
                : 'Apertura de Servicio (Inicial)'}
            </h3>
            <button
          ...
          {showAddItem && (
            <form
              ...
              className="mb-6 p-4 bg-gray-50 dark:bg-black/20 rounded-lg flex gap-3 items-end border border-gray-200 dark:border:white/5 animate-slide-up"
            >
              ...
              <button
                type="submit"
                className="bg-sushi-gold text-sushi-black p-2 rounded hover:bg-sushi-goldhover"
              >
          ...
          <div className="space-y-4">
            {items.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-4 py-2 border-b border-gray-100 dark:border-white/5 last:border-0"
              >
                <div className="flex-1">
        ...
        <div className="space-y-6">
          ...
          {sessions
            ...
            .map((session) => (
              <div
                key={session.id}
                className="bg-white dark:bg-sushi-dark border border-gray-200 dark:border:white/5 rounded-xl overflow-hidden shadow-sm"
              >
                <div className="bg-gray-50 dark:bg-black/20 p-4 border-b border-gray-200 dark:border:white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    ...
                    <div className="flex gap-4 mt-2 text-xs text-gray-500 dark:text-sushi-muted">
                      <span className="flex items-center gap-1">
                        <Clock className="w-3 h-3" /> Apertura: {session.startTime} (
                        {session.openedBy})
                      </span>
                      <span className="flex items-center gap-1">
                        <Clock className="w-3 h-3" /> Cierre: {session.endTime} (
                        {session.closedBy})
                      </span>
                    </div>
                  </div>
                  ...
                  >
                    {generatingReportId === session.id ? (
                      ...
                      <Sparkles className="w-4 h-4 text-sushi-gold" />
                    )}
                ...
                {reportText && generatingReportId === null && (
                  <div className="p-4 bg-yellow-50 dark:bg-sushi-gold/5 border-b border-yellow-100 dark:border-sushi-gold/10 relative">
                    <h4 className="text-xs font-bold uppercase text-yellow-700 dark:text-sushi-gold mb-2">
                      Borrador Generado
                    </h4>
                ...
                <div className="p-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                  {session.data.map((d) => {
                    ...
                    return (
                      <div
                        key={d.itemId}
                        className="bg-gray-50 dark:bg-white/[0.02] p-2 rounded border border-gray-100 dark:border:white/5"
                      >
                        <p className="text-xs text-gray-500 dark:text-sushi-muted uppercase">
                          {item.name}
                        </p>
                        <p className="font-mono font-bold text-gray-900 dark:text:white">
                          -{(d.consumption ?? 0).toFixed(1)}{' '}
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-white dark:bg-sushi-dark px-2 text-gray-400">
                    O manual
                  </span>
                </div>
              </div>
